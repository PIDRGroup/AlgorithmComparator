...
\begin{algorithm}[H]
 \KwData{RETOUR : structure de donnée de deux type (Booléen et Double) décrivant réciproquement si la recherche est un échec et la taille du chemin, limite : valeur que l'on s'impose à chaque recherche, Parcours : tableau contenant le plus chemin pour atteindre les noeuds déjà étudiés du graphe, Noeuds : liste de tous les noeuds du graphe, Source : nœud source, Destination : nœud destination}
 \KwResult{Parcoursfinal : Liste ordonnée du plus court chemin de la Source à la Destination}
...
\Fn{RBFS(Listenoeud, noeud, limite)}{
	...
	\While{true}{
		...
		//On observe si une deuxième solution est possible ou non\linebreak
